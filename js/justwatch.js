...
// ---------------------------------------------------------------------------
// Core fetch helper — StreamFinder pattern:
// 1) try local /api/jw (Vercel proxy), 2) fall back to public CORS proxies
//    so detail page still works on file:// / live-server without /api/jw.
// ---------------------------------------------------------------------------
const PROXIES = [
...
async function fetchViaProxy(operationName, query, variables) {
    const maxRetries = PROXIES.length;
    let attempts = 0;
    while (attempts < maxRetries) {
        const proxyUrl = PROXIES[currentProxyIndex];
        const apiUrl = `${proxyUrl}/https://apis.justwatch.com/graphql`;
        try {
            ...
            if (data.errors?.length) {
                console.error('JustWatch GraphQL errors (proxy):', data.errors);
                return null;
            }
...
async function jwRequest(operationName, query, variables) {
    // 1) Preferred: Vercel serverless proxy (no CORS, no rate-limit on client).
    // NOTE: no custom User-Agent — browsers forbid it and the fetch would fail.
    try {
        ...
        console.warn(`JustWatch proxy ${JW_API} -> HTTP ${res.status}, trying CORS fallback...`);
    } catch (err) {
        console.warn("JustWatch /api/jw unreachable, trying CORS fallback:", err?.message || err);
    }

    // 2) Fallback: StreamFinder-style rotating CORS proxies (local dev / static host).
    return fetchViaProxy(operationName, query, variables);
}

// ---------------------------------------------------------------------------
// Build per-country offer aliases.
// Country values must be GraphQL enum literals (no quotes).
// StreamFinder pattern: bestOnly + preAffiliate for smaller, faster response.
// ---------------------------------------------------------------------------
function buildCountryOfferAliases(countries) {
    return countries.map(c => `
        ${c.toLowerCase()}: offers(country: ${c}, platform: WEB, filter: { bestOnly: true, preAffiliate: true }) {
            ...OfferFields
        }
    `).join("\n");
}
